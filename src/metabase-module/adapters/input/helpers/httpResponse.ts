...
export class LambdaHttpResponse {
	public static success(
		...
		if (isFile) {
			return {
				...
				statusCode: 200,
				body: (payload as Buffer)?.toString('base64') ?? ({} as Buffer).toString('base64'),
				headers: {
					...headers,
	...
	public static error(statusCode: number, code?: string, message?: string): APIGatewayProxyResult {
		if (statusCode < 500) {
			console.warn(
